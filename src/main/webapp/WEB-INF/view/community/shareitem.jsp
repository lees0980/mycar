...
<main id="center">
   <div class="notice margin-top">
            ...
            <c:forEach var="n" items="${list}">
               <tr>
                  ...
                  <td class="title indent text-align-left"><a
                     href="shareitem/${n.id}">${n.title}</a></td>
                  <td>${n.writer}</td>
	...
	<div class="indexer margin-top align-right">
      <h3 class="hidden"></h3>
      <div>
         <span class="text-orange text-strong">${param.p}</span> / ${shareItemPage} pages
      </div>
   </div>
   </div>
   <form class="table-form">
      <fieldset>
         ...
         <input type="button" onclick="location.href='regShareItem'" value="글쓰기" />
      </s:authorize>
      <s:authorize access="isAnonymous()">

            <input class="btn-reg" type="button"  onclick="location.href='/member/login'" value="글쓰기"/>
        </s:authorize>
      </fieldset>
   </form>
	<ul class="pager">
          <c:forEach var="i" begin="1" end="${shareItemPage}"> 
